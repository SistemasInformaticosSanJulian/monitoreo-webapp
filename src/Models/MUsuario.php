...
class MUsuario
{
    ...
    public function setRequest(array $data): void
    {
        ...
        $this->rol = \array_key_exists('rol', $data) ? trim($data['rol']) : 'cliente';
    }

    public function save(): array
    {
        if ($this->id !== 0) {
            $query = 'update usuario set nombre=?, contrasenia=?, rol=?, updated_at=? where id=?;';
            $values = [$this->nombre, $this->passwordHash($this->contrasenia), $this->rol, date('Y-m-d H:i:s'), $this->id];
        ...
        $values = [$this->nombre, $this->passwordHash($this->contrasenia), $this->rol, date('Y-m-d H:i:s')];
